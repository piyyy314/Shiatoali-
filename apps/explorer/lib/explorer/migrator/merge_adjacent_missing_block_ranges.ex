    ...
    {priority_ranges, non_priority_ranges, delete_ids} =
      Enum.reduce(ranges_batch, {[], [], []}, fn range, {priority_acc, non_priority_acc, delete_acc} ->
        if is_nil(range.priority) do
          {priority_acc, [range.from_number..range.to_number//-1 | non_priority_acc], [range.id | delete_acc]}
        else
          {[range.from_number..range.to_number//-1 | priority_acc], non_priority_acc, [range.id | delete_acc]}
        end
      end)
